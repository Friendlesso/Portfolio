...
export function GitHubWindow({ item, onClose }: GitHubProps) {
  if (!item) return
  return (
    <div className="dual-border-folder p-1 bg-[var(--folder-background)] flex flex-col h-[80vh] w-sm sm:w-2xl lg:w-5xl ">
      <WindowHeader label={item.label} icon={item.icon} bgColor={item.headerColor} onClose={onClose} />
      <div className="bg-[var(--github-background)] px-4 sm:px-6 lg:px-12 overflow-y-auto text-white">
        <GithubProfile />
        <GitHubRepos />
